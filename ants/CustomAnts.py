...
def show_header_info(path):
	...
	print(
		f"Header Information for: {os.path.basename(path)} \n Number of Dimensions: {header_info['nDimensions']} \n Dimensions: {header_info['dimensions']} \n Spacing: {header_info['spacing']}"
	)
...
def save_plot(image,path,overlay=None,Title=None):
	filename = f'{os.getcwd()}/plots/{os.path.basename(path)}.png'
	if overlay is None:
...
if ants.get_orientation(fixed_antsimage) != ants.get_orientation(moving_antsimage):
	print(
		f'Re-orienting moving image to fixed image. {ants.get_orientation(moving_antsimage)} -> {ants.get_orientation(fixed_antsimage)}'
	)
...
fixed_antsimage.plot(
	overlay=moving_antsimage,
	cmap='Reds',
	overlay_cmap='jet',
	title='Before Registration',
	filename=f"{os.getcwd()}/plots/Before_registration.png",
)

fixed_antsimage.plot(
	overlay=warped_moving,
	cmap='Reds',
	overlay_cmap='jet',
	title='After Registration',
	filename=f"{os.getcwd()}/plots/After_registration.png",
)
